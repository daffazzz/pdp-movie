...
function TVShowGenrePageContent() {
  const searchParams = useSearchParams();
  const genreIdString = searchParams?.get('id') as string;
  ...
  useEffect(() => {
    const fetchShowsByGenre = async () => {
      ...
      setHasMore(true);
      try {
        const [genreData, countriesData, showsData] = await Promise.all([
          fetchFromTMDB('genre/tv/list'),
          fetchFromTMDB('configuration/countries'),
          fetchFromTMDB('discover/tv', `with_genres=${genreId}&page=1${countryCode ? `&with_origin_country=${countryCode}` : ''}`),
        ]);
  ...
  useEffect(() => {
    const loadMoreShows = async () => {
      ...
      try {
        const showsData = await fetchFromTMDB('discover/tv', `with_genres=${genreId}&page=${page}${countryCode ? `&with_origin_country=${countryCode}` : ''}`);
...
export default function TVShowGenrePage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-background pt-24 px-4 flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-red-600"></div></div>}>
      <TVShowGenrePageContent />
    </Suspense>
  );
}
