...
const Hero: React.FC<HeroProps> = ({
  ...
  if (!hasContent) {
    return (
      <div className="relative h-[100vh] w-full overflow-hidden bg-[#1a1a1a]">
  ...
  return (
    <div ref={heroRef} className="relative h-[100vh] w-full overflow-hidden hero-container">
      ...
      <div
        className={`absolute h-full w-full transition-opacity duration-500 ease-in-out z-0 ${isLoaded && !showTrailer ? 'opacity-100' : 'opacity-0'
          }`}
      >
      ...
      {!isLoaded && (
        <div className="absolute inset-0 bg-[#1a1a1a] flex items-center justify-center z-[15]">
          <div className="w-12 h-12 border-t-4 border-red-500 border-solid rounded-full animate-spin"></div>
        </div>
      )}
      ...
      {/* Content */}
      <div
        className={`absolute bottom-[30%] z-[20] transition-all duration-300 container mx-auto px-4 xl:px-8 2xl:px-16 mb-10 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
          }`}
      >
